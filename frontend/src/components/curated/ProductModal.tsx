...
import {
  normalizeFragranceDisplayText,
  normalizeSubjectParticle,
  splitFragranceDisplayName,
} from "@/utils/koreanText";
...
export default function ProductModal({ product, onClose }: ProductModalProps) {
  const displayNameParts = splitFragranceDisplayName(product.name);
  const displayName = displayNameParts.displayName;
  const story = normalizeSubjectParticle(
    normalizeFragranceDisplayText(product.details.story),
    displayName,
  );
      ...
      <div className="relative bg-cream w-full max-w-5xl max-h-[90vh] overflow-y-auto rounded-sm shadow-2xl animate-in zoom-in-95 fade-in duration-500 flex flex-col md:flex-row">
        ...
        <div className="md:w-1/2 p-8 md:p-16 flex flex-col text-wood">
          <div className="mb-10">
            ...
            <h3
              className="text-3xl md:text-4xl font-light tracking-tight mb-4 break-keep [word-break:keep-all] [overflow-wrap:normal] [line-break:strict]"
          ...
          <div className="mb-12">
            ...
            <p className="text-[15px] leading-relaxed break-keep [word-break:keep-all] [overflow-wrap:normal] [line-break:strict] text-pretty font-light italic text-wood/80">
              "{story}"
            </p>
          </div>
